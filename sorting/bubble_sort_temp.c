...
int main() {
  ...
  for (int i = 0; i < n - 1; i++) { // for n sized array, sorting procedure
                                    // needs to be done n - 1 times
    for (int j = 0; j < n - 1 - i;
         j++) { // traversing the array until 2nd last element; (j + 1) takes
                // care of last element; ignore bubbled element (i) at last
